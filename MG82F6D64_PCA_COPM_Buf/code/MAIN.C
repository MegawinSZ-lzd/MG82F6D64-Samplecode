...
void InitPort(void)
{
	...
	PORT_SetP2PushPull(BIT2|BIT4);						// 设置P22为推挽输出
}


/***********************************************************************************
函数名称:   void InitPCA_COPM_Buffer(void)
功能描述:   PCA初始化
		PCA时钟: 12MHz
...
void InitPCA_COPM_Buffer(void)
{
	...
	PCA_CH0_SetMode_COPM();			// 设置CH0 为COPM Mode
	PCA_CH1_SetMode_COPM();			// 设置CH0 为COPM Mode
	PCA_CH2_SetMode_COPM();			// 设置CH2 为COPM Mode
	PCA_CH3_SetMode_COPM();			// 设置CH0 为COPM Mode
	...
	// 使能PWM输出
	PCA_SetPWM0_EnOutput();						// 使能CEX0 PWM0输出
	PCA_SetPWM2_EnOutput();						// 使能CEX2 PWM2输出
