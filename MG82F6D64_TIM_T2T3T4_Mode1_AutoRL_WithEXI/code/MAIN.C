/*********************************************************************
    项目名称:MG82F6D64-DEMO
		...
		功能描述：
			定义T2为Mode1:自动重载与EXI,时钟为Sysclk/12	
			定义T3为Mode1:自动重载与EXI,时钟为Sysclk/12  
			定义T4为Mode1:自动重载与EXI,时钟为Sysclk/12	
		注意事项：
...
{
	TM_SetT2Mode_AutoRLwithExI();			// 设置T2模式1为16位自动重载与外部中断模式
...
{
	TM_SetT3Mode_AutoRLwithExI();			// 设置T3模式1为16位自动重载与外部中断模式
...
{
	TM_SetT4Mode_AutoRLwithExI();			// 设置T4模式0为16位自动重载与外部中断模式
