...
void InitPort(void)
{
	...
	PORT_SetP0AInputOnly(BIT1|BIT2);  					// 设置P01,P02仅输入，做CMP输入通道
}

/***********************************************************************************
函数名称:   void InitCMP(void)
功能描述:   CMP初始化
输入参数:   
返回参数:     
*************************************************************************************/
...
{
	AC0_Enable();										// 设置使能CMP0
	AC0_SetInterruptMode_Rising();						// 设置中断模式：上升沿中断
	AC0_SetPosInputP02();								// 设置CMP0正极输入P02
	AC0_SetNegaReferenceVoltage_AC0NI();				// 设置CMP0负极输入P01
	AC0_DisHysteresis();								// 设置CMP0禁止迟滞控制
}
