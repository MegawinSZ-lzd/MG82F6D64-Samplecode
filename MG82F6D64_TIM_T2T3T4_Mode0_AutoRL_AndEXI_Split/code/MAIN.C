/*********************************************************************
    项目名称:MG82F6D64-DEMO
		...
		功能描述：
			定义T2为模式0的分割模式,时钟为Sysclk/12	
			定义T3为模式0的分割模式,时钟为Sysclk/12  
			定义T4为模式0的分割模式,时钟为Sysclk/12  
		注意事项：
...
{
	TM_SetT3Mode_AutoRLandExI();			// 设置T3模式0为8位自动重载和外部中断模式
...
{
	TM_SetT4Mode_AutoRLandExI();			// 设置T4模式0为8位自动重载和外部中断模式
