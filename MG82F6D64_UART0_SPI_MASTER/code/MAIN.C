...
/***********************************************************************************
函数名称:   u8 Uart0SPITransceiver(u8 ToSend)
功能描述:   串口SPI主机发送函数
输入参数:   u8 ToSend
返回参数:     
*************************************************************************************/
...
/***********************************************************************************
函数名称:   void InitUart0_SPI_Master(void)
功能描述:   Uart0初始化 SPI Master
输入参数:   
返回参数:     
*************************************************************************************/
...
{
	P31=1;
	UART0_SetModeSPIMaster();			// 设置串口SPI主机模式
	UART0_SetMISOP16();					// 设置串口SPI的MISO引脚为P16
	UART0_SetRxTxP30P31();				// 设置串口SPI的RX（MOSI）,TX(SPI_CLK)引脚为P30,P31
#if (SPI_CLOCK == SPI_CLK_SYSCLK_4)
	UART0_SetSPIClock_SYSCLKDiv4();		// 设置SPI时钟为系统时钟的4分频
#else
	UART0_SetSPIClock_SYSCLKDiv12();	// 设置SPI时钟为系统时钟的12分频
#endif
...
void InitPort(void)
{
	...
	PORT_SetP3QuasiBi(BIT0|BIT1);						// 设置P30,P31为准双向口
	PORT_SetP1OpenDrainPu(BIT6);						// 设置P16,做MISO
	PORT_SetP3PushPull(BIT3);							// 设置P33为推挽输出(NSS)
}
