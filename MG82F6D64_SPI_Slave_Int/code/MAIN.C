/*********************************************************************
	...
	功能描述：
			SPI(nSS/P14,MOSI/P15,MISO/P16,SPICLK/P17)从机模式
			中断方式接收
	注意事项：
...
void InitPort(void)
{
	...
	PORT_SetP1PushPull(BIT6);							// 设置P16(MISO)为推挽输出 
	PORT_SetP1OpenDrainPu(BIT4);						// 设置P14(nss) 为准双向口
	PORT_SetP1OpenDrain(BIT5|BIT7);						// 设置 P15(MOSI),P17(SPICLK) 为开漏，用于SPICLK/MOSI			
}

/***********************************************************************************
函数名称:   void InitSPI_Slave(void)
功能描述:   SPI初始化 slave
输入参数:   
返回参数:     
*************************************************************************************/
void InitSPI_Slave(void)
{
	...
	SPI_Enable();									// 使能SPI
	SPI_SelectMASTERSLAVEBynSSpin();				// 设置nSS引脚选择MASTER/SLAVE
	SPI_SetClock(SPI_CLK_SYSCLK_4);					// 设置SPI时钟 SYSCLK/4 12M/4=3M
	SPI_SetCPOL_0();								// 设置SPICLK初始电平 CPOL=0 低电平
	SPI_SetDataMSB();								// 设置数据传送位序 MSB 高位在前
	SPI_SetCPHA_0();								// 设置采样和改变数据模式 CPHA=0 前沿采样,后沿改变数据
