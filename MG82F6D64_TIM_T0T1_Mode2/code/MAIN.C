/*********************************************************************
    项目名称:MG82F6D64-DEMO
		...
		功能描述：
			定义T0为8位定时器,时钟为Sysclk/192	
			定义T1为8位定时器,时钟为Sysclk/48
		注意事项：
...
void INT_T0(void) interrupt INT_VECTOR_T0
{
	...
	LED_G_0=!LED_G_0;			//TF0在进入中断时会被硬件自动清掉，因此不需要if(TF0)
}
...
{
	LED_G_1=!LED_G_1;			//TF1在进入中断时会被硬件自动清掉，因此不需要if(TF1)
}
...
{
	TM_SetT0Mode_2_8BIT_AUTORELOAD();		// 设置T0模式为8位模式
	TM_SetT0Clock_SYSCLKDiv192();			// 设置T0时钟源为 SYSCLK/192
	TM_SetT0Gate_Disable();

	TM_SetT0LowByte(0);						// 设置T0低8位
	TM_SetT0HighByte(200);					// 设置T0高8位作为重载值
